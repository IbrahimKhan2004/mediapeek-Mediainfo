...
/** not-@type {!BigUint64Array} */
  ...
  async function instantiateAsync(binary, binaryFile, imports) {
    if (Module['wasmModule']) {
      ...
      return { instance, module: Module['wasmModule'] };
    }
  ...
  function createJsInvoker(argTypes, isClassMethodFunc, returns, isAsync) {
    ...
    return function (
      ...
      var toClassParamWire = restArgs[0];
      var argsOffset = 1;

      var argCount = argTypes.length - 2;
      var argConverters = restArgs.slice(argsOffset, argsOffset + argCount);
      var explicitDestructors = restArgs.slice(argsOffset + argCount);

      var accessDestructor = {};
      if (!needsDestructorStack) {
        let dtorIndex = 0;
        let startIdx = isClassMethodFunc ? 1 : 2;
        for (var i = startIdx; i < argTypes.length; ++i) {
          if (argTypes[i].destructorFunction !== null) {
            accessDestructor[i] = explicitDestructors[dtorIndex++];
          }
        }
      }
      ...
      return function (...funcArgs) {
        ...
        if (isClassMethodFunc) {
          thisWired = toClassParamWire(dtorStack, this);
          wiredArgs.push(thisWired);
        }

        var wiredParams = {};
        for (var i = 0; i < argCount; ++i) {
          var val = argConverters[i](dtorStack, funcArgs[i]);
          wiredArgs.push(val);
          wiredParams[i + 2] = val;
        }
        if (isClassMethodFunc) {
          wiredParams[1] = thisWired;
        }

        var rv = invoker.apply(null, wiredArgs);
        ...
        if (needsDestructorStack) {
          ...
          let startIdx = isClassMethodFunc ? 1 : 2;
          for (var i = startIdx; i < argTypes.length; ++i) {
            if (accessDestructor[i]) {
              accessDestructor[i](wiredParams[i]);
            }
          }
        }
